							   ...
							   LONG * plAveCharWidth, LONG * plAveCharHeight, CONST HWND hMainWnd )
{
    ...
    arrHwnd[ID_BTN_STOP]			    = CreateWindow(TEXT("BUTTON"), TEXT("S"), BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD | WS_TABSTOP| BS_ICON | BS_CENTER, 0, 0, 0, 0, hMainWnd, (HMENU)ID_BTN_STOP, g_hInstance, NULL);
	SendMessage(arrHwnd[ID_BTN_STOP],BM_SETIMAGE,IMAGE_ICON,(LPARAM)LoadImage(g_hInstance,MAKEINTRESOURCE(IDI_ICON_STOP),IMAGE_ICON,16,16,LR_DEFAULTCOLOR|LR_SHARED));
...
VOID DisplayStatusOverview(CONST HWND hEditStatus)
{
	...
	if(SyncQueue.dwCountNoCrcFound > 0){
		StringCchPrintf(szLineTmp, MAX_LINE_LENGTH, TEXT(" %ux 未找到 HASH,"), SyncQueue.dwCountNoCrcFound);
		StringCchCat(szLine, MAX_LINE_LENGTH, szLineTmp);
	}
...
VOID UpdateOptionsDialogControls(CONST HWND hDlg, CONST BOOL bUpdateAll, CONST PROGRAM_OPTIONS * pprogram_options)
{
	...
	CheckDlgButton(hDlg, IDC_CLOSE_AFTER_SHELLEXT_ACTION, pprogram_options->bCloseAfterActionFromShellExt ? BST_CHECKED : BST_UNCHECKED);
